...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    input_tensors[0].append(
        p2p_communication.recv_forward(tensor_shape, config, is_vp_first_stage(vp_stage=0))  # 获取chunk 0的输入，但是如果chunk 0的输入直接是训练数据，那么还是由data iterator提供
    )
    ...
    for k in range(num_warmup_microbatches):
        ...
        if config.overlap_p2p_comm_warmup_flush:
            if not is_vp_first_stage(vp_stage=cur_model_chunk_id) and k != 0:  # 非第一个chunk
    ...
    for k in range(num_microbatches_remaining):
        ...
        if config.overlap_p2p_comm:
            if not is_vp_first_stage(vp_stage=cur_model_chunk_id): # 非第一阶段，第一阶段的数据不是从上游接收的
                if config.overlap_p2p_comm_warmup_flush:
                    assert recv_prev_wait_handles, (
            ...
            if not is_vp_last_stage(vp_stage=backward_model_chunk_id):  # 不是bwd的第一阶段，需要等待接收数据
    ...
    if not forward_only:
        ...
        for k in range(num_microbatches_remaining, total_num_microbatches):  # bwd剩余数量和fwd warmup的数量相同，起始id为 num_warmup_microbatches
            ...
            if not is_vp_last_stage(vp_stage=cur_model_chunk_id) and k != 0:  # 非bwd的第一stage，需要接收数据
                if config.overlap_p2p_comm_warmup_flush:
                    assert recv_next_wait_handles, (
